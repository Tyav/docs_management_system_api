...
router.get('/', [ tokenAuth, adminAuth ], async (req, res) => {
	...
	const users = await User.find({}).select('_id username email name createdAt roleId modifiedAt');
...
router.post('/', async (req, res) => {
	...
	res.status(201).header('x-auth-token', token).send(_.pick(user, [ '_id', 'username', 'email', 'roleId', 'name', 'createdAt', 'modifiedAt'  ]));
});
...
router.put('/:id', [ idAuth, tokenAuth ], async (req, res) => {
	...
	const token = req.header('x-auth-token')
	res.status(200).header('x-auth-token', token).send(_.pick(updatedUser, [ '_id', 'username', 'email', 'name' ]));
});
...
router.delete('/:id', [ tokenAuth, loginAuth ], async (req, res) => {
	...
	const token = req.header('x-auth-token')
	res.status(200).header('x-auth-token', token).send({ Success: 200, message: 'User deleted' });
});
