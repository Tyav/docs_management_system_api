...
router.get('/', [ tokenAuth, adminAuth ], async (req, res) => {
	...
	const users = await User.find({ deleted: deleted }).select('_id username email name createdAt roleId modifiedAt verified');
...
router.get('/:id', [ tokenAuth, idAuth ], async (req, res) => {
	...
	res.status(200).header('x-auth-token', token).send(_.pick(user, [ '_id', 'username', 'email', 'name', 'createdAt', 'roleId', 'modifiedAt', 'verified' ]));
});
...
router.post('/', async (req, res) => {
	...
	res.status(201).header('x-auth-token', token).send(_.pick(user, [ '_id', 'username', 'email', 'roleId', 'name', 'createdAt', 'modifiedAt' , 'verified']));
});
...
router.post('/login', [ ifLogin ], async (req, res) => {
	...
	res.status(200).header('x-auth-token', token).send(_.pick(user, [ '_id', 'username', 'email', 'roleId', 'name', 'createdAt', 'modifiedAt' , 'verified']));
});
...
router.put('/:id', [ idAuth, tokenAuth ], async (req, res) => {
	...
	const token = req.header('x-auth-token');
	res.status(200).header('x-auth-token', token).send(_.pick(updatedUser, [ '_id', 'username', 'email', 'roleId', 'name', 'createdAt', 'modifiedAt' , 'verified']));
});
